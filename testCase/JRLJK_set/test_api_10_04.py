...
class Test_api_10_04(unittest.TestCase):
    '''
    用例名称：输入正确的炉子编号以及错误密码无法拿到相应的数据
    用例描述：输入正确的炉子编号以及错误密码无法拿到相应的数据
    用例编号：api_10_04
    '''
    ...
    def test_Case(self):
        ...
        if  "errcode" and "errmsg" in  self.data_text[1].keys():
            ...
            return False
        #错误码
        if int(self.data_text[1]['errcode']) == 103:
            Logger().logger.info("errcode  == 103")
        else:
            Logger().logger.info("errcode != 103")
            Logger().logger.info(str(Path(__file__).name + " : " + self.File))
            return False

        if self.data_text[1]['errmsg'] == "密码错误":
