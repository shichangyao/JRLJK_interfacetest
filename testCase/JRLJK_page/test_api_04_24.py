...
class Test_api_04_24(unittest.TestCase):
    '''
    ...
    def setUp(self):
        ...
        self.pasrms_furnaceNo = ReadConfig().get_pasrms("furnaceNo")[8]
    ...
    def test_Case(self):
        ...
        if self.data_text[1]['errcode'] == 101:
            ...
            Logger().logger.info(str(Path(__file__).name + " : " + self.File))
            return False
        if self.data_text[1]['errmsg'] == "区域编号必须为整数!":
            Logger().logger.info("区域编号必须为整数!")
        else:
            Logger().logger.info("区域编号不为整数!")
            Logger().logger.info(str(Path(__file__).name + " : " + self.File))
            return False
