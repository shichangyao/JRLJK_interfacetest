...
class Test_api_04_16(unittest.TestCase):
    '''
    用例名称：请求使用超出数据库统计的炉子编号和区域编号不可以正常得到温度数据
    用例描述：使用超出数据库统计的炉子编号和区域编号进行请求会提示错误信息
    用例编号：api_04_16
    接口路径：/api/now_timing/
    '''
    ...
    def setUp(self):
        ...
        self.pasrms_areaNo = ReadConfig().get_pasrms("areaNo")[12]
    ...
    def test_Case(self):
        ...
        if self.data_text[1]['errmsg'] == "不存在的炉子编号11!":
            Logger().logger.info("不存在的炉子编号11!")
        else:
            Logger().logger.info("存在的炉子编号11!")
            Logger().logger.info(str(Path(__file__).name + " : " + self.File))
            return False
