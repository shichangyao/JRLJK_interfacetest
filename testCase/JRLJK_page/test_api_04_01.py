...
class Test_api_04_01(unittest.TestCase):
    '''
    用例名称：请求使用正确的炉子编号和区域编号以及sign值为0和dur_time的值可以正常得到最近时间段温度数据
    用例描述：使用正确的炉子编号和区域编号以及sign值为0和dur_time的值进行请求可以得到正确的响应并且得到相应数据
    用例编号：api_04_01
    '''
    ...
    def setUp(self):
        ...
        self.sign = ReadConfig().get_pasrms("sign")[0]
        self.dur_time = ReadConfig().get_pasrms("dur_time")[0]
