...
class Test_api_04_17(unittest.TestCase):
    '''
    用例名称：请求使用正确的炉子编号和超出数据库数据的区域编号不可以正常得到温度数据
    ...
    def setUp(self):
        ...
        self.pasrms_areaNo = ReadConfig().get_pasrms("areaNo")[13]
    ...
    def test_Case(self):
        ...
        if self.data_text[1]['errcode'] == 101:
            ...
            Logger().logger.info(str(Path(__file__).name + " : " + self.File))
            return False
        if self.data_text[1]['errmsg'] == "区域7在2号炉中不存在!":
            Logger().logger.info("区域7在2号炉中不存在!")
        else:
            Logger().logger.info("区域7在2号炉中存在!")
            Logger().logger.info(str(Path(__file__).name + " : " + self.File))
            return False
