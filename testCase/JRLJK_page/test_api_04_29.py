...
class Test_api_04_29(unittest.TestCase):
    '''
    用例名称：请求使用错误的炉子编号和正确的区域编号不可以正常得到温度数据
    用例描述：使用错误的炉子编号和正确的区域编号进行请求会提示错误信息
    用例编号：api_04_29
    '''
    ...
    def setUp(self):
        ...
        self.pasrms_furnaceNo = ReadConfig().get_pasrms("furnaceNo")[10]
        self.pasrms_areaNo = ReadConfig().get_pasrms("areaNo")[11]
        self.pasrms = {"furnaceNo":self.pasrms_furnaceNo,"areaNo":self.pasrms_areaNo}
    ...
    def test_Case(self):
        ...
        if self.data_text[1]['errcode'] == 101:
            ...
            Logger().logger.info(str(Path(__file__).name + " : " + self.File))
            return False
        if self.data_text[1]['errmsg'] == "未收到炉子编号！":
            Logger().logger.info("errmsg：未收到炉子编号！")
        else:
            Logger().logger.info("errmsg ：返回值错误")
            Logger().logger.info(str(Path(__file__).name + " : " + self.File))
            return False
