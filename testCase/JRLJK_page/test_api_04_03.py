...
class Test_api_04_03(unittest.TestCase):
    '''
    用例名称：请求使用正确的炉子编号和区域编号以及sign值为0和正确的dur_time的值和interval的值可以得到最近时间段温度数据（interval取值为10的公倍数）
    用例描述：使用正确的炉子编号和区域编号以及sign值为0和正确的dur_time的值和interval的值进行请求可以得到正确的响应并且得到相应数据
    用例编号：api_04_03
    '''
    ...
    def setUp(self):
        ...
        self.dur_time = ReadConfig().get_pasrms("dur_time")[1]
