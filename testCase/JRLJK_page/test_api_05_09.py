...
class Test_api_05_09(unittest.TestCase):
    '''
    用例名称：请求使用正确的炉子编号和页码以及时间范围和采样间隔可以得到相应时间段的数据（时间使用毫秒级的时间戳）
    用例描述：请求使用正确的炉子编号和页码以及时间范围和采样间隔可以正常响应并得到相应时间段的数据
    用例编号：api_05_09
    '''
    ...
    def setUp(self):
        ...
        self.pasrms = {"furnaceNo":self.pasrms_furnaceNo,
                       "start_time":"1587542400000",
                       "end_time":"1587600000000",
