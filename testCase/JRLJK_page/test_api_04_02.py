...
class Test_api_04_02(unittest.TestCase):
    '''
    用例名称：请求使用正确的炉子编号和区域编号以及错误的dur_time的值可以正常得到最近时间段温度数据（sign不填写时默认是0）
    用例描述：使用正确的炉子编号和区域编号以及错误的dur_time的值进行请求可以得到正确的响应并且得到相应数据
    用例编号：api_04_02
    '''
