...
class Test_api_04_09(unittest.TestCase):
    '''
    用例名称：请求使用正确的炉子编号和区域编号以及sign值为1和dur_time的值不能正常得到最近时间段温度数据
    用例描述：使用正确的炉子编号和区域编号以及sign值为1和dur_time的值进行请求可以不能得到相应数据
    用例编号：api_04_09
    '''
    ...
    def setUp(self):
        ...
        self.dur_time = ReadConfig().get_pasrms("dur_time")[6]
