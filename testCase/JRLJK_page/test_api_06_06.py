...
class Test_api_06_06(unittest.TestCase):
    '''
    用例名称：请求使用特殊数据的炉子编号不可以正常得到数据
    用例描述：请求使用特殊数据的炉子编号不可以正常得到数据
    用例编号：api_06_06
    '''
    ...
    def test_Case(self):
        ...
        if self.data_text[1]['errcode'] == 101:
            Logger().logger.info("errcode 返回值验证成功")
        else:
            Logger().logger.info("errcode 返回值验证失败")
            Logger().logger.info(str(Path(__file__).name + " : " + self.File))
            return False
