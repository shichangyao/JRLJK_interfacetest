...
class Test_api_02_02(unittest.TestCase):
    '''
    用例名称：请求使用特殊值的炉子编号不能正常得到温度数据
    用例描述：使用特殊值的炉子编号不能得到正确的响应
    用例编号：api_02_02
    '''
    ...
    def test_Case(self):
        ...
        if self.data_text[1]['errcode'] == 101  and self.data_text[1]['errmsg']  == '不存在的炉子编号0!':
            Logger().logger.info("errcode/errmsg返回信息验证成功")
        else:
            Logger().logger.info("errcode/errmsg返回信息验证失败")
            Logger().logger.info(str(Path(__file__).name + " : " + self.File))
            return False
