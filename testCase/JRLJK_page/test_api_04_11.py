...
class Test_api_04_11(unittest.TestCase):
    '''
    用例名称：请求使用正确的炉子编号和区域编号以及sign值为1且输入start_time和
                end_time的范围能正常得到最近时间段温度数据（最大24小时，使用毫秒级的时间戳）
    用例描述：使用正确的炉子编号和区域编号以及sign值为1，且输入start_time和end_time的范围进行请求可以得到相应数据
    用例编号：api_04_11
    '''
    ...
    def setUp(self):
        ...
        self.start_time = "1587600000000"
        self.end_time ="1587603600000"
