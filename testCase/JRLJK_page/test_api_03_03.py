...
class Test_api_03_03(unittest.TestCase):
    '''
    用例名称：请求使用特殊值的炉子编号不能正常得到温度数据和工艺参数
    用例描述：请求使用特殊值的炉子编号不能正常得到温度数据和工艺参数
    用例编号：api_03_03
    '''
    ...
    def test_Case(self):
        ...
        if  "errcode" and "errmsg" in  self.data_text[1].keys():
            ...
            Logger().logger.info(str(Path(__file__).name + " : " + self.File))
            return False
        if self.data_text[1]['errcode'] == 101 and self.data_text[1]['errmsg'] == "不存在的炉子编号0!":
            Logger().logger.info("errcode/errmsg 字段值验证成功")
        else:
            Logger().logger.info("errcode/errmsg 字段值验证失败")
            Logger().logger.info(str(Path(__file__).name + " : " + self.File))
            return False
