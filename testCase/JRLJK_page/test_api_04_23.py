...
class Test_api_04_23(unittest.TestCase):
    '''
    用例名称：请求使用特殊字符的炉子编号和区域编号不可以正常得到温度数据
    用例描述：使用特殊字符的炉子编号和区域编号进行请求会提示错误信息
    用例编号：api_04_23
    '''
    ...
    def setUp(self):
        ...
        self.pasrms_areaNo = ReadConfig().get_pasrms("areaNo")[9]
    ...
    def test_Case(self):
        ...
        if self.data_text[1]['errcode'] == 101:
            ...
            Logger().logger.info(str(Path(__file__).name + " : " + self.File))
            return False
        if self.data_text[1]['errmsg'] == "不存在的炉子编号】!":
            Logger().logger.info("不存在的炉子编号】!")
        else:
            Logger().logger.info("存在的炉子编号】!")
            Logger().logger.info(str(Path(__file__).name + " : " + self.File))
            return False
