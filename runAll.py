...
class AllTest:
    def __init__(self):#初始化一些参数和数据
        ...
        resultPath = os.path.join(report_path, "report"+nowTime+".html")#result/report.html
    ...
    def run(self):
        ...
        try:
            ...
            if suit is not None:#判断test_suite是否为空
                ...
                #调用HTMLTestRunner
                runner = HTMLTestRunner.HTMLTestRunner(stream=fp, title='接口测试报告', description='Test Description',verbosity=2)
                log.info(runner)
                runner.run(suit)
                fp.close()
            else:
